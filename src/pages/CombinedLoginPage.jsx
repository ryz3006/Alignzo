...
const TabButton = ({ active, onClick, children }) => (
    <button type="button" onClick={onClick} style={{flex: 1, padding: '1rem', fontWeight: '600', borderBottom: '4px solid transparent', transition: 'all 0.3s ease', color: 'var(--light-text)'}} className={active ? 'active-tab' : ''}>
        <style>{`.active-tab { border-color: var(--light-primary); color: var(--light-primary); } html.dark .active-tab { border-color: var(--dark-primary); color: var(--dark-primary); }`}</style>
        {children}
    </button>
);
    ...
    <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '1rem'}}>
      <div className="neumorph-outset" style={{width: '100%', maxWidth: '420px', padding: '2rem'}}>
        <div style={{textAlign: 'center', marginBottom: '2rem'}}>
            ...
            <p style={{marginTop: '0.5rem'}}>Smarter Alignment. Smarter Operations.</p>
        </div>
        
        <div style={{display: 'flex', marginBottom: '1.5rem'}} className="neumorph-inset">
        ...
        <div>
            {activeTab === 'user' ? (
                <button onClick={handleGoogleLogin} disabled={loading} className="btn neumorph-outset" style={{width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                ...
                <form onSubmit={handleAdminLogin} style={{display: 'flex', flexDirection: 'column', gap: '1.5rem'}}>
                  ...
                  <button type="submit" disabled={loading} className="btn neumorph-outset bg-primary" style={{color: 'white'}}>
